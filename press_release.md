...
In the scatterplot above, we can see primary productivity values over time. The dotted red line marks the separation between the train and test set. Specifically, the last 15% of the data was reserved for the test set, since this is a forecasting problem. We can see there are no obvious trends in primary productivity over time just looking at this plot, so we need more in-depth data analysis to help us better understand trends in primary productivity.
